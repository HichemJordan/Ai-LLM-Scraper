...
if st.session_state["scraping_state"] == "waiting":
    ...
    with st.spinner("Scraping in progress..."):
        ...
        output_folder = os.path.join(
            "output", generate_unique_folder_name(st.session_state["urls"][0])
        )
        ...
        if st.session_state["attended_mode"] and driver is not None:
            ...
            if st.session_state["use_pagination"]:
                pagination_data, token_counts, pagination_price = (
...
if st.session_state["scraping_state"] == "completed" and st.session_state["results"]:
    ...
    if pagination_info:
        ...
        st.dataframe(
            pagination_df,
            column_config={"Page URLs": st.column_config.LinkColumn("Page URLs")},
    ...
    if show_tags and pagination_info:
        st.markdown("---")
        total_input_tokens_combined = (
            total_input_tokens + pagination_info["token_counts"]["input_tokens"]
        )
        total_output_tokens_combined = (
            total_output_tokens + pagination_info["token_counts"]["output_tokens"]
        )
